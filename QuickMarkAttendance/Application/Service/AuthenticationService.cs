...
using QuickMarkAttendance.Application.SQRS.DoctorFeature.CreateDoctor;
using QuickMarkAttendance.Application.SQRS.Student.CreateStudent;
using QuickMarkAttendance.Application.SQRS.StudentFeature.CheckIp;
...
namespace QuickMarkAttendance.Application.Service
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public AuthenticationService(UserManager<IdentityUser<Guid>> userManager, IOptions<JwtSettings> options, IUnitOfWork unitOfWork, RoleManager<IdentityRole<Guid>> roleManager, IMediator mediator)
        {
        ...
        public async Task<Result<JwtTokenDto>> Register(string Username, string Email, string Password, string Role,string? ip)
        {
            try
            {
                ...
                if (existingUser != null)
                {
                    ...
                    if ( roles.Count() !=0 && roles.FirstOrDefault() == Role)
                        return Result.Error("Username is already in use");
                }

                if (Role == "Student")
                {
                    var chechIp = await _mediator.Send(new CheckIpQuery(ip));
                    if (chechIp.Value != null) return Result.Error("This ip is already exist");
                ...
                if (!string.IsNullOrEmpty(Role))
                {
                    ...
                    if (Role == "Student")
                    {
                        var createResult = await _mediator.Send(new CreateStudentCommand(newUser.UserName,newUser.Id));
                    }

                    if (Role == "Doctor")
                    {
                        var createResult = await _mediator.Send(new CreateDoctorCommand(newUser.Id,newUser.UserName));
                    }
